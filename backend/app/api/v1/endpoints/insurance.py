...
async def delete_insurance_policy(
    ...
    db.query(Alert).filter(Alert.asset_id == policy_id).update(
        {Alert.asset_id: None}, synchronize_session=False
    )
    db.query(AssetSnapshot).filter(AssetSnapshot.asset_id == policy_id).update(
        {AssetSnapshot.asset_id: None}, synchronize_session=False
    )
    db.query(MutualFundHolding).filter(MutualFundHolding.asset_id == policy_id).delete(
        synchronize_session=False
    )
    db.query(Transaction).filter(Transaction.asset_id == policy_id).delete(
        synchronize_session=False
    )
