...
const SWPSetup: React.FC = () => {
  ...
  const refreshPlans = async () => {
    try {
      const res = await mfPlansAPI.getAll('swp');
      setPlans(res.plans || []);
    } catch (err) {
      notify.error(getErrorMessage(err));
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingPlanId) {
        ...
        notify.success('SWP plan updated');
      } else {
        await mfPlansAPI.create(payload);
        notify.success('SWP plan created');
      }
      setDialogOpen(false);
      refreshPlans();
    } catch (err) {
  ...
  const handleDelete = async (planId: number) => {
    if (!window.confirm('Delete this SWP plan?')) return;
    try {
      await mfPlansAPI.delete(planId);
      notify.success('SWP plan deleted');
      refreshPlans();
    } catch (err) {
      notify.error(getErrorMessage(err));
    }
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h5">SWP Setup</Typography>
        <Button variant="contained" startIcon={<AddIcon />} onClick={openCreateDialog}>
          Add SWP
        </Button>
      </Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Set up Systematic Withdrawal Plans for your mutual funds. The system will automatically
        create SELL transactions on the scheduled dates to redeem units at current NAV.
      </Typography>

      {plans.length === 0 ? (
        <Alert severity="info" sx={{ mb: 2 }}>
          No SWP plans configured yet. Click the + button to add your first SWP.
        </Alert>
      ) : (
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingPlanId ? 'Edit SWP Plan' : 'New SWP Plan'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <TextField
              ...
              fullWidth
              InputLabelProps={{ shrink: true }}
              helperText="Leave blank for perpetual SWP"
            />
