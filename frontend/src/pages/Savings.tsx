...
const Savings: React.FC = () => {
  ...
  const handleDelete = async (account: BankAccount) => {
    if (!window.confirm(`Delete ${formatBankName(account.bank_name)} (••••${account.account_number.slice(-4)})? All expenses linked to this account will also be permanently deleted. This cannot be undone.`)) {
      return;
    }
    try {
