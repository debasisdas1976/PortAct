...
const STPSetup: React.FC = () => {
  ...
  const refreshPlans = async () => {
    try {
      const res = await mfPlansAPI.getAll('stp');
      setPlans(res.plans || []);
    } catch (err) {
      notify.error(getErrorMessage(err));
    }
  ...
  const handleSubmit = async () => {
    if (!form.asset_id || !form.target_asset_id || !form.amount || !form.start_date) {
      notify.error('Please fill in all required fields');
      return;
    }
    if (form.asset_id === form.target_asset_id) {
      notify.error('Source and target fund must be different');
      return;
    }
    try {
      setSubmitting(true);
      ...
      if (editingPlanId) {
        ...
        notify.success('STP plan updated');
      } else {
        await mfPlansAPI.create(payload);
        notify.success('STP plan created');
      }
      setDialogOpen(false);
      refreshPlans();
    } catch (err) {
  ...
  const handleDelete = async (planId: number) => {
    if (!window.confirm('Delete this STP plan?')) return;
    try {
      await mfPlansAPI.delete(planId);
      notify.success('STP plan deleted');
      refreshPlans();
    } catch (err) {
      notify.error(getErrorMessage(err));
    }
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h5">STP Setup</Typography>
        <Button variant="contained" startIcon={<AddIcon />} onClick={openCreateDialog}>
          Add STP
        </Button>
      </Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Set up Systematic Transfer Plans to automatically transfer amounts between mutual funds.
        The system will create SELL on the source fund and BUY on the target fund at current NAV.
      </Typography>

      {plans.length === 0 ? (
        <Alert severity="info" sx={{ mb: 2 }}>
          No STP plans configured yet. Click the + button to add your first STP.
        </Alert>
      ) : (
        <TableContainer component={Paper} sx={{ mb: 2 }}>
          <Table size="small">
            <TableHead>
              <TableRow>
                <TableCell>Source Fund</TableCell>
                <TableCell>Target Fund</TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingPlanId ? 'Edit STP Plan' : 'New STP Plan'}</DialogTitle>
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <TextField
              select
              label="Source Fund (Transfer From)"
              value={form.asset_id}
            ...
            <TextField
              select
              label="Target Fund (Transfer To)"
              value={form.target_asset_id}
              onChange={(e) => setForm({ ...form, target_asset_id: Number(e.target.value) })}
              fullWidth
              required
            >
              {mfAssets
                .filter((a) => a.id !== form.asset_id)
                .map((a) => (
                  <MenuItem key={a.id} value={a.id}>
                    {a.name}
                  </MenuItem>
            ...
            <TextField
              ...
              fullWidth
              InputLabelProps={{ shrink: true }}
              helperText="Leave blank for perpetual STP"
            />
