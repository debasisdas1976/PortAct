...
const ASSET_TYPE = 'nsc';
const PAGE_TITLE = 'National Savings Certificate (NSC)';
...
const NSC: React.FC = () => {
  ...
  const handleEdit = (asset: AssetItem) => {
    ...
    setForm({
      name: asset.name || '',
      symbol: asset.symbol || '',
      total_invested: String(asset.total_invested || ''),
      current_value: String(asset.current_value || ''),
      interest_rate: String(asset.details?.interest_rate || ''),
      maturity_date: asset.details?.maturity_date || '',
      broker_name: asset.broker_name || '',
      notes: asset.notes || '',
    });
    setDialogOpen(true);
  };
              ...
              assets.map((asset) => (
                <TableRow key={asset.id} hover>
                  <TableCell>
                    ...
                    {asset.broker_name && <Typography variant="caption" color="text.secondary">{asset.broker_name}</Typography>}
                  </TableCell>
