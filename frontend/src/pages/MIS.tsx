...
const ASSET_TYPE = 'mis';
const PAGE_TITLE = 'Monthly Income Scheme (MIS)';
...
const MIS: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    const payload = { asset_type: ASSET_TYPE, name: form.name, symbol: form.symbol || undefined, total_invested: invested, current_value: value, quantity: 1, purchase_price: invested, current_price: value, broker_name: form.broker_name || undefined, notes: form.notes || undefined, details };
    try {
      setSaving(true);
  ...
  return (
    <Box>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={6} md={4}><Card><CardContent><Typography color="text.secondary" variant="body2">Accounts</Typography><Typography variant="h4">{assets.length}</Typography></CardContent></Card></Grid>
      ...
      <TableContainer component={Paper}>
        <Table>
          <TableHead><TableRow>
            ...
            <TableCell align="right"><strong>Interest Rate</strong></TableCell><TableCell align="right"><strong>Monthly Payout</strong></TableCell>
            <TableCell><strong>Maturity Date</strong></TableCell><TableCell align="center"><strong>Actions</strong></TableCell>
          </TableRow></TableHead>
          <TableBody>
            {assets.length === 0 ? (
              ...
              const monthlyPayout = asset.details?.interest_rate && asset.total_invested ? (asset.total_invested * asset.details.interest_rate / 100 / 12) : null;
              return (
                <TableRow key={asset.id} hover>
                  ...
                  <TableCell align="right">{monthlyPayout ? formatCurrency(monthlyPayout) : '-'}</TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            ...
            <Grid item xs={12} sm={6}><TextField fullWidth label="Account No." value={form.symbol} onChange={(e) => setForm({ ...form, symbol: e.target.value })} /></Grid>
